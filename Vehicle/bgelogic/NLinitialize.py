...
def _initialize(owner):
    ...
    ACT0003 = bgelogic.InitEmptyDict()
    ACT0004 = bgelogic.ActionSetGameObjectGameProperty()
    ACT0005 = bgelogic.ActivateActuatorByName()
    CON0006 = bgelogic.ConditionOnInit()
    ...
    ACT0004.game_object = "Object:car"
    ACT0004.property_name = "_args"
    ACT0004.property_value = ACT0003.DICT
    ACT0005.condition = ACT0002.OUT
    ACT0005.actuator = "FXAA"
    ...
    owner["initialize"] = network
    network._owner = owner
    network.setup()
    network.stopped = not owner.get('NODELOGIC__initialize')
    return network

def pulse_network(controller):
    owner = controller.owner
    network = owner.get("initialize")
    if network is None:
